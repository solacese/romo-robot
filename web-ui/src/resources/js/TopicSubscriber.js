        ...
        subscriber.session.on(solace.SessionEventCode.MESSAGE, function (message) {
            ...
            if (characterCommand[0].highestEmotion != undefined) {
                ...
                if (parseInt(characterCommand[0].higestConfidence) > 65) {
                    document.getElementById('last' + emotionName).src = "https://s3.us-east-2.amazonaws.com/jdiamond-reinvent/" + characterCommand[0].image;
                } else {
                    console.log("Emotion not high enough: " + characterCommand[0].higestConfidence);
                }
    ...
    subscriber.publish = function (command) {
        if (subscriber.session !== null) {
            ...
            switch(command.toUpperCase()){
                case 'MOVE_FORWARD':
                    romoMoveMessage = {driveForwardWithSpeed:{speed:0.5, duration:0.5}};
                    break;
                case 'MOVE_BACKWARD':
                    romoMoveMessage = {driveBackwardWithSpeed:{speed:0.5, duration:0.5}};
                    break;
                case 'ROTATE_UP':
                    romoMoveMessage ={turnByAngle:{speed:0.5,radius:0,angle:10}};
                    break;
                case 'ROTATE_DOWN':
                    romoMoveMessage = {turnByAngle:{speed:0.5,radius:0,angle:-10}};
                    break;
            ...
            message.setDestination(solace.SolclientFactory.createTopicDestination(romoMovementTopicName));
            message.setBinaryAttachment(romoMoveMessageText);
            message.setDeliveryMode(solace.MessageDeliveryModeType.DIRECT);
            subscriber.log('Publishing message "' + romoMoveMessageText + '" to topic "' + romoMovementTopicName + '"...');
            try {
            ...
            subscriber.log('Cannot publish because not connected to Solace message router.');
        }
    };
